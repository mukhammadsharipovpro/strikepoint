...
import StrikePointCartItemComponent from '../components/StrikePointCartItemComponent';
...
export default function () {
  ...
  const handleOrder = () => {
    const destinationScreen = cart.length
      ? 'StrikePointCartSuccessScreen'
      : 'StrikePointHomeScreen';
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <StrikePointCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
